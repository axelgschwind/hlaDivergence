...
def scatterplot_pairwise_metrics(prefix = "", loci = ["all", "A", "B", "C", "DQB1", "DRB1"]):
	global data_
	...
	for locus in loci:
		...
		for i, (metric1, metric2) in enumerate(metrics, start=1):
			...
			data_per_locus.plot( x=metric1, y=metric2, kind="scatter", s=1, ax=ax1, rasterized=True)
		...
		fig.savefig("%sHLA-%s_pairwise_correlation.pdf" % (prefix, locus), dpi=1200)
...
def histogram(data: pd.DataFrame, prefix = "", loci = ["all", "A", "B", "C", "DQB1", "DRB1"], metrics = ["PDISTANCE", "JTT", "DAYHOFF", "GRANTHAM", "SANDBERG"]):
	...
	for locus in loci:
		...
		for i, metric in enumerate(metrics, start=1):
			...
			ax.hist(data_per_locus[metric], bins = 30, density=True, edgecolor='black', linewidth=1, rasterized=True)
		...
		fig.savefig("%s%s_divergence_histograms.pdf" % (prefix,locus), dpi=1200)
...
def main(argv):
	...
	(options, loci) = parser.parse_args()
	...
	if len(loci) > 0:
		scatterplot_pairwise_metrics(options.prefix, loci)
		histogram(data_, options.prefix, loci)
	else:
		scatterplot_pairwise_metrics(options.prefix)
